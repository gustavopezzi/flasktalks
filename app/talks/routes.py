...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    pagination = user.talks.order_by(Talk.date.desc()).paginate(page, per_page=current_app.config['TALKS_PER_PAGE'], error_out=False)
    talk_list = pagination.items
    return render_template('talks/user.html', user=user, talks=talk_list, pagination=pagination)
